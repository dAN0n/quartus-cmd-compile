...
SET BATCH_DIR=%~dp0
SET TCL_SCRIPT=%BATCH_DIR%quar.tcl
...
pushd %PROJECT_DIR%
%QUARTUS_DIR%\quartus_sh -t %TCL_SCRIPT% -project %PROJECT_NAME% -sv "%PROJECT_FILES%" %QUARTUS_COMPILE% %QUARTUS_ARCHIVE% %PROJECT_MISC_FILES%
...
IF NOT EXIST "%MS_PROJECT_DIR%" mkdir %MS_PROJECT_DIR%
IF NOT EXIST "%MS_SRCDIR%" mkdir %MS_SRCDIR%

:: COPY SV/DO FILES
IF "%OPEN_EXISTING_PROJECT%"=="" FOR %%I in (%PROJECT_FILES%) do copy %%I %MS_SRCDIR%
IF NOT "%PROJECT_MISC_FILES%"=="" FOR %%I in (%PROJECT_MISC_FILES%) do copy %%I %MS_SRCDIR%
...
pushd %MS_PROJECT_DIR%
IF EXIST "%PROJECT_NAME%.mpf" %MODELSIM_DIR%\vsim -do "do %TCL_SCRIPT:\=/% -modelsim %PROJECT_NAME% {%PROJECT_FILES%} {%PROJECT_MISC_FILES%}"
IF NOT EXIST "%PROJECT_NAME%.mpf" %MODELSIM_DIR%\vsim -do "project new . %PROJECT_NAME%; do %TCL_SCRIPT:\=/% -modelsim %PROJECT_NAME% {%PROJECT_FILES%} {%PROJECT_MISC_FILES%}"
