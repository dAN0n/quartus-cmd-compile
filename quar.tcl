...
proc generate_do {name} {
    ...
    puts $tcl "#"
    puts $tcl "# Use \"virtual signal {<signals>} <name>\" for creating custom groups (not adding to wave)"
    puts $tcl "# For example: \"virtual signal {sw\[7:4\]} sw7_4\""
    puts $tcl "#"
    ...
    puts $tcl "# In that situations use: foreach signal \[find signals -internal *\] { force \$signal 0 -cancel 0 }"
    puts $tcl "#"
    ...
    puts $tcl "#"
    puts $tcl "# For decrement sequence use:"
    puts $tcl "# set t 0; for {set i 19} {\$i >= 0} {incr i -1} { force sw 8'd\$i \[expr \$t*10\]; incr t }"
    puts $tcl "#"
    puts $tcl "# For random sequence use:"
    puts $tcl "# for {set i 0} {\$i < 20} {incr i} { force sw 8'd\[expr int(rand()*1000)\] \[expr \$i*10\] }"
    puts $tcl "#"
    puts $tcl "# For running simulation use \"run \[@\]<time>\" or \"run -all\""
    puts $tcl "#"
    ...
    puts $tcl "# Use \"restart -f -nowave\" for restart simulation"
    puts $tcl ""
    puts $tcl ""
    puts $tcl "restart -f -nowave"
    puts $tcl "view wave"
    puts $tcl "add wave *"
    puts $tcl ""
    puts $tcl "foreach signal \[find signals -internal *\] { force \$signal 0 -cancel 0 }"
    puts $tcl ""
    puts $tcl "run -all"
